...
public class AStarPathfinder {
	...
	public static void findPath(int x1, int y1, int x2, int y2){
		...
		openList = new PriorityQueue<TileInfo>(superList.length * superList[0].length, new ComparatorByScore());
		...
		superList[x1][y1] = new TileInfo(x1, y1, null, 0, getHeuristicScore(x1, y1, x2, y2));
	...
	public static void calcSurroundingTile(TileInfo tile, int cX, int cY){
		...
		if (newX < 0 || newY < 0 || newX >= Map.map.passable.length || newY >= Map.map.passable[0].length){
			return;
		}

		//Now we take that surrounding tile from the SUPER LIST.
		TileInfo surroundingTile = superList[tile.tileX + cX][tile.tileY + cY];
		...
		int toTileCost = (cX != 0 && cY != 0 ? 14 : 10) * Map.map.passCost[newX][newY];
		...
		if (surroundingTile == null){
			...
			surroundingTile = new TileInfo(newX, newY, tile, tile.cost + toTileCost, getHeuristicScore(newX, newY, destX, destY));
